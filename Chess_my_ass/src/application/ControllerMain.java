...
public class ControllerMain implements Initializable, EventHandler {
	...
	private Stage stage;
	private Scene scene;
	private String startScene = "StartScreen.fxml";

	public void switchStartScreen(ActionEvent event) throws IOException {
		Parent root = FXMLLoader.load(getClass().getResource(startScene));
	...
	private void figurenForPlayersReset() {
		...
		for (int i = listpositionblack; i >= 0; i--) {
			paneZelle = (Pane) gridPaneGitterBlack.lookup("#paneZelleB_" + i);
			// if (paneZelle != null)
			paneZelle.getChildren().clear();
		}
		listpositionblack = 0;
